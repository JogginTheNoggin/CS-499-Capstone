...
private:
	/* ADDED for future reference, will be turned into a enum with id*/
	enum shape { CUBE, CONE, CYLINDER, PLANE, PRISM, PYRAMID3, PYRAMID4, SPHERE, TAPEREDCYLINDER, TORUS };
	...
	void drawShape(shape shape, glm::vec3 scale, glm::vec3 rotation, glm::vec3 posTrans, glm::vec4 color);
	void drawShape(shape shape, glm::vec3 scale, glm::vec3 rotation, glm::vec3 posTrans, std::string textureName); // texture version of draw shape
...
public:
	...
	void renderWall(glm::vec3 posTranslate, glm::vec3 offset, float angle );
