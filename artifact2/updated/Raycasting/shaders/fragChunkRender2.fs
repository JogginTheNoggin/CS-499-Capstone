...
uniform float startX; // world coordinate X start of texture
uniform float startY; // world coordinate Y start of texture
uniform float startZ; // world coordinate Z start of texture
...
// should be using local coordinates to texture
...
bool getVoxel(ivec3 mapPos) {
	ivec3 adjustedPos = ivec3(mapPos.x - startX, mapPos.y - startY, mapPos.z - startZ); //(no abs) adjust to texture coordinates 0 - textAxisLength, by using the start coordinates of current chunks
	return (mapPos.x == 0 && mapPos.y == 0 && mapPos.z == -12) || sample(adjustedPos);
...
void main(){
	...
	for (int i = 0; i < MAX_DISTANCE; i++) {
		...
		else if(( mapPos.y < startY && rayDir.y < 0 )|| (mapPos.y > startY + textHeight && rayDir.y > 0.0)){ // voxels past maxHeight are empty by default, stop raycasting if ray pointing further upwards
			break; // ray looking into space
			// can add one for outofbounds too
		}
		else if(( mapPos.x < startX && rayDir.x < 0 )|| ( mapPos.x > startX + textWidth && rayDir.x > 0.0) ){ // voxels past maxHeight are empty by default, stop raycasting if ray pointing further upwards
			break; // ray looking into space
			// can add one for outofbounds too
		}
		else if(( mapPos.z < startZ && rayDir.z < 0 ) || (mapPos.z > startZ + textDepth && rayDir.z > 0.0)){ // voxels past maxHeight are empty by default, stop raycasting if ray pointing further upwards
			break; // ray looking into space
			// can add one for outofbounds too
		}
